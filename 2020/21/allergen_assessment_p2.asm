; https://adventofcode.com/2020/day/21 (part 2)

    OPT listoff : DEVICE ZXSPECTRUMNEXT : MMU 0 7, 0, $0000
inputFile:  ; load the input.txt into virtual device memory for processing
;     INCBIN "input_t.txt"    ; expected result: mxmxvkd,sqjhc,fvjkl
    INCBIN "input.txt"      ; expected result: vv,nlxsmb,rnbhjk,bvnkk,ttxvphb,qmkz,trmzkcfg,jpvz
inputEnd:
...
part2Output:
    DS      MAX_ALLERGENS * 100             ; max 100 char per line of output
...
inPtr = inPtr + 2
    ...
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ; PART 2 change:
    ; produce also output file with allergen -> ingredient to make easy to build final answer in text editor
    ; (too lazy to code actualy sorting)
...
w2 = {dictionary + (wordId<<2)}
                    ...
                    ; produce output also into memory for final dump into text file
                    ORG p2Out : DB '['
                        WHILE 'a' <= {b w1} && {b w1} <= 'z'
                            DB {b w1}
w1 = w1 + 1
...
wordId = wordId + 1
    ...
    SAVEBIN "p2out.txt", part2Output, p2Out - part2Output

    ; to compose final answer for web, open the file in text editor (for example Kate)
    ; use Sort function to sort lines by allergen ("Tools -> Scripts -> Editing -> Sort..." in Kate)
    ; use regex search+replace to keep ingredients as comma separated list:
    ;   search regex: ^\[\w+\] (\w+)\n
    ;   replace with: \1,
    ; And delete the extra comma at the end  of the line
    ; (done including this description in 5min, instead of scripting it for 30min)
